  ...
  try {
    ...
    for (const [key, value] of Object.entries(req.query)) {
      switch (key) {
        case "king":
          searchQuery.$or = [
            { attacker_king: { $regex: `.*${value}.*`, $options: "i" } },
            { defender_king: { $regex: `.*${value}.*`, $options: "i" } },
          ];
          break;
        case "type":
          searchQuery.battle_type = value;
          break;
        case "location":
          searchQuery.location = value;
          break;
        default:
          break;
      }
  ...
  res.json({
    ...
    endpoints: [
      ...
      },
      {
        url: "/search",
        desc:
          "get the data with all applied filters for king,type and location",
        method: "GET",
      },
    ],
  });
